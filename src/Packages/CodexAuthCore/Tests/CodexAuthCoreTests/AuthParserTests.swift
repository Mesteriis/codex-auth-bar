...
struct AuthParserTests {
    ...
    @Test func rejectsAuthFilesLargerThanTenMiBBeforeParsing() {
        let oversized = Data(repeating: 0x20, count: AuthParser.maximumBytes + 1)

        #expect(throws: AuthError.authFileTooLarge) {
            try AuthParser.parse(oversized)
        }
    }
}
