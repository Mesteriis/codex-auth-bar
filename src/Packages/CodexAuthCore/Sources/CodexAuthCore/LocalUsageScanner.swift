...
public enum LocalUsageScanner {
    public static func newest(home: CodexHome, activatedAtMilliseconds: Int64?) throws -> LocalUsageEvent? {
        ...
        guard let newest,
              let text = String(data: try SecureFiles.readRegularFile(newest, maximumBytes: 128 * 1024 * 1024), encoding: .utf8)
        else { return nil }
        ...
        for line in text.split(separator: "\n") {
            ...
            guard let timestamp = timestampMilliseconds(root["timestamp"] ?? payload["timestamp"]) else { continue }
    ...
    private static func timestampMilliseconds(_ value: Any?) -> Int64? {
        if let number = value as? NSNumber {
            return number.int64Value > 10_000_000_000 ? number.int64Value : number.int64Value * 1_000
        }
