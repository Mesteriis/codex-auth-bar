...
public actor ProfileStore {
    ...
    public func list() throws -> [ProfileName] {
        ...
        return try FileManager.default.contentsOfDirectory(
            ...
            .compactMap { url -> ProfileName? in
                ...
                guard url.lastPathComponent.hasSuffix(suffix), isRegularProfile(url) else { return nil }
    ...
    public func create(_ name: ProfileName) throws {
        ...
        var status = stat()
        guard lstat(url.path, &status) != 0 else { throw ProfileError.alreadyExists }
    ...
    public func rename(_ old: ProfileName, to new: ProfileName) throws {
        ...
        guard isRegularProfile(source) else { throw ProfileError.notFound }
        var status = stat()
        guard lstat(destination.path, &status) != 0 else { throw ProfileError.alreadyExists }
        try FileManager.default.moveItem(at: source, to: destination)
    }

    public func delete(_ name: ProfileName) throws {
        let url = profileURL(name)
        guard isRegularProfile(url) else { throw ProfileError.notFound }
        try FileManager.default.removeItem(at: url)
    }

    public func profileURL(_ name: ProfileName) -> URL { home.root.appending(path: "\(name.rawValue).config.toml") }

    private func isRegularProfile(_ url: URL) -> Bool {
        var status = stat()
