...
Первым публичным бинарным релизом станет `v0.1.0-rc.1`: universal DMG,
подписанный Developer ID, notarized и stapled. Он появится в
[GitHub Releases](https://github.com/Mesteriis/codex-auth-bar/releases).

Пока Apple signing credentials не настроены, приложение собирается из исходного
кода. Unsigned CI-артефакты предназначены только для разработки и намеренно не
публикуются как релизы.
